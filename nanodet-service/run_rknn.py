...
def post_process(preds, scale_factor=1, rescale=False):
        ...
        for stride, anchors in zip(strides, mlvl_anchors):
                cls_score, bbox_pred = preds[ind:(ind + anchors.shape[0]), :num_classes], preds[
                                                                                          ind:(ind + anchors.shape[0]),
                                                                                          num_classes:]

                cls_score = np.array(1 / (1 + np.exp(-cls_score)))
                ind += anchors.shape[0]
...
srcimg = cv2.imread(r'/workspaces/rknn-toolkit/nanodet-main/111111111.jpg').astype(np.float32)
...
# 调用 inference 接口进行推理
outputs = rknn.inference(inputs=[img],data_type='float32',data_format="nchw")[0][0]
t1 =time.time()
det_bboxes, det_conf, det_classid = post_process(outputs)
t2 =time.time()
print('------------------------------------------------------------------------------------',t2-t1)
...
cv2.imwrite(r'/workspaces/rknn-toolkit/nanodet-service/result_img.jpg',result_img)
