...
class OneStageDetector(nn.Module):
    ...
    def inference(self, meta):
        with torch.no_grad():
            # # onnx 测试
            # sess = onnxruntime.InferenceSession(r'H:\pycharmproject\nanodet-main\nanodet.onnx')
            # input_name = sess.get_inputs()[0].name
            # preds = sess.run(None, {input_name: np.array(meta["img"])})[0]
            # preds = torch.tensor(preds)

            torch.cuda.synchronize()
            time1 = time.time()
            preds = self(meta["img"])
            m1 = torch.jit.load(r'H:\pycharmproject\nanodet-main\nanodet-plus-m_416_torchscript.pth')
            out = m1(meta["img"])[0]
            torch.cuda.synchronize()
            time2 = time.time()
